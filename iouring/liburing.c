...
int push_read_request(int file_fd, off_t file_sz) {
  ...
  struct file_info *fi =
      malloc(sizeof(*fi) + (sizeof(struct iovec) * total_blocks));
  // Populate iovecs.
  for (int i = 0; i < total_blocks; i++) {
...
int queue_submit(int num) { return io_uring_submit_and_wait(&ring, num); }

int queue_init() { return io_uring_queue_init(QUEUE_DEPTH, &ring, 0); }

void queue_exit() { io_uring_queue_exit(&ring); }
